    ...
    for mos_type in mos_list:
        for thres in thres_list:
            for lch in lch_list:
                ...
                if mos_type == 'nch':
                    tb_obj.set_parameter('vgs_start', 0.3)
                    tb_obj.set_parameter('vgs_stop', 1.2)
                    tb_obj.set_sweep_parameter('vds', start=0.0, stop=1.2, step=0.1)
                else:
                    tb_obj.set_parameter('vgs_start', -1.2)
                    tb_obj.set_parameter('vgs_stop', -0.3)
                    tb_obj.set_sweep_parameter('vds', start=-1.2, stop=0.0, step=0.1)
...
def compute_ss_helper(mos_type, results, output):
    # get sweep variables name and order
    xvar_list = results['sweep_params'][output]
    xname, yname = xvar_list
    # outer sweep variable 1D array
    xvec = results[xname]
    # inner sweep variable 1D array
    yvec = results[yname]
    ...
    fun = RectBivariateSpline(xvec, yvec, zmat)
    if xname == 'vds':
        gm = fun(xvec, yvec, dy=1, grid=True)
        gds = fun(xvec, yvec, dx=1, grid=True)
    else:
        gds = fun(xvec, yvec, dy=1, grid=True)
        gm = fun(xvec, yvec, dx=1, grid=True)
    
    vstar = 2 * ibmat / gm
    return dict(gm=gm, gds=gds, vstar=vstar)
